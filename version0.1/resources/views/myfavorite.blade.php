...
<div class="masonry-wrapper">
    <div class="container">
        <ul class="grid effect-2" id="grid">
          ...
          @foreach($quotedata1 as $quote)      
          <li>

              <a href="{{url('poster/'.$quote->quotedata->id)}}">
                <img class="w-100" src="{{URL::asset('images/quotes/'.$quote->quotedata->image)}}">
              </a>
              <div class="masonry-quotes-box">
                  <div class="masonry-quotes-icon">
                      <div class="heart">
                        @if(!empty(Session::get('user')) && $user=Session::get('user'))
                        @if(!empty($fav) && in_array($quote->quotedata->id, $fav))
                        <span id="myHeart" onclick="changeHeart({{$quote->quotedata->id}});"><i class="fa fa-heart active" aria-hidden="true"></i></span>
                        @else
                        <span id="myHeart" onclick="changeHeart({{$quote->quotedata->id}});"><i class="fa fa-heart" aria-hidden="true" ></i></span>
                        @endif
                     @else
                     ...
                     @endif
                      ...
                      @if(!empty(Session::get('user')) && $user=Session::get('user'))
                        @if(!empty($coll) && in_array($quote->quotedata->id, $coll))
                        <span id="myPlus" class="quotelink" onclick="changePlus({{$quote->quotedata->id}});"><i class="fa fa-plus-circle active" aria-hidden="true"></i></span>
                        @else
                        <span id="myPlus" class="quotelink" onclick="changePlus({{$quote->quotedata->id}});"><i class="fa fa-plus-circle" aria-hidden="true"></i></span>
                        @endif
                      @else
                  ...
                  <div class="masonry-content">
                    ...
                    <a  class="quotelink" href="{{url('people-innerside/'.$quote->quotedata->people->id)}}"><h6>{{$quote->quotedata->people->name}}</h6></a>
                  </div><!-- /.masonry-content -->

                  <div class="masonry-related-links">
                        @php
                        $i=1;
                        $value=count($quote->quotedata->topics);
                        @endphp
                        @foreach($quote->quotedata->topics as $relatedtopic)
